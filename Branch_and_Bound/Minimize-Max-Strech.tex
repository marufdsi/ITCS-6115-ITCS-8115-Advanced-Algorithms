...
\begin{equation*}
\begin{array}{l}
S\ = P! \qquad\qquad\text{Where } P=\{p_1,\dots,p_n\}
\end{array}
\end{equation*}
We can partition the set $S$ into $n$ subsets where,
\begin{equation*}
\begin{array}{l}
S\ = S_1\cup S_2\cup \dots\cup S_n \qquad\qquad \text{Where }S_i = \{p_{i1},\dots,p_{in}\}\ \text{and }S_i\subset  S | S_{p_1}=p_i
\end{array}
\end{equation*}
...
\begin{equation*}
\begin{array}{l}
\text{explore }S_i \ \text{branch before } S_j\ \text{if }  r_{i1}\leq r_{j1} 
\end{array}
\end{equation*}
...
If two process has the same release time then always schedule the job that has smaller process time. If $r_i=r_j=t_r$ and $p_i<p_j$ then if we schedule
$p_i$ first at start time $t_{\sigma}$, 
\begin{equation}
\begin{array}{l}
Stretch_{i1} = \frac{t_{\sigma} + p_i - t_r}{p_i} \\
Stretch_{j1} = \frac{t_{\sigma} + p_i + p_j- t_r}{p_j} 
\end{array}
\end{equation}
Now if schedule $p_j$ before $p_i$
\begin{equation}
\begin{array}{l}
Stretch_{j2} = \frac{t_{\sigma} + p_j - t_r}{p_j} \\
Stretch_{i2} = \frac{t_{\sigma} + p_i + p_j- t_r}{p_i} 
\end{array}
\end{equation}
Here, $Stretch_{j2} < Stretch_{j1}$ but $Stretch_{i2}$ is greater than both $Stretch_{i1}$ and $Stretch_{j1}$. So, Schedule the smaller job with 
